...
def criar_municipio_completo(
    ...
    db_session.merge(
        Estado(id=estado_id, sigla=estado_sigla, nome=f"Estado {estado_sigla}", regiao="Nordeste")
    )
...
def test_listar_municipios_filtra_por_estado(client, db_session):
    ...
    criar_municipio_completo(db_session, 3550308, "São Paulo", 35, "SP")
...
def test_listar_municipios_ordena_e_limita(client, db_session):
    criar_municipio_completo(db_session, 2507507, "João Pessoa", 25, "PB", populacao=800000)
    criar_municipio_completo(db_session, 3550308, "São Paulo", 35, "SP", populacao=12000000)
    criar_municipio_completo(db_session, 3106200, "Belo Horizonte", 31, "MG", populacao=2500000)
